...
func NewCustomCommandHandler(github *ghclient.Client, workspace *workspace.Manager, sessionManager *code.SessionManager, mcpClient mcp.MCPClient, globalConfigPath string, codeProvider string) *CustomCommandHandler {
...
func (h *CustomCommandHandler) CanHandle(ctx context.Context, githubCtx models.GitHubContext) bool {
	...
	log.Println("githubCtx", githubCtx)

	// Check if global config path is available
	if h.globalConfigPath == "" {
		xl.Infof("V6 handler disabled - no global config path")
		return false
	}
	...
	xl.Infof("V6 handler can process command: %s", cmdInfo.Command)
	return true
}
...
func (h *CustomCommandHandler) buildGitHubEvent(ctx context.Context, githubCtx models.GitHubContext, instruction string) (*githubcontext.GitHubEvent, error) {
	...
	xl.Infof("Built GitHub event with %d issue comments, %d PR comments, %d review comments, %d changed files",
...
func (h *CustomCommandHandler) createWorkspaceForEvent(ctx context.Context, githubCtx models.GitHubContext, aiModel string) (*models.Workspace, error) {
	...
	case *models.IssueCommentContext:
		if ctx.IsPRComment {
			...
			pr, err := h.github.GetPullRequest(repo.GetOwner().GetLogin(), repo.GetName(), ctx.Issue.GetNumber())
			if err != nil {
...
func (h *CustomCommandHandler) collectPRChangedFiles(ctx context.Context, pr *github.PullRequest) ([]string, error) {
	...
	// Note: The h.github field contains our GitHub client which wraps the GitHub API client
	files, _, err := h.github.GetClient().PullRequests.ListFiles(ctx, owner, repoName, prNumber, &github.ListOptions{
...
func (h *CustomCommandHandler) collectIssueCommentHistory(ctx context.Context, issueCtx *models.IssueCommentContext) ([]string, error) {
	...
	issueComments, _, err := h.github.GetClient().Issues.ListComments(ctx, owner, repoName, issueNumber, &github.IssueListCommentsOptions{
	...
	xl.Infof("Collected %d comments for Issue #%d", len(comments), issueNumber)
	return comments, nil
}
...
func (h *CustomCommandHandler) collectPRCommentHistory(ctx context.Context, prCtx interface{}) (prComments, reviewComments []string, err error) {
	...
	prIssueComments, _, err := h.github.GetClient().Issues.ListComments(ctx, owner, repoName, prNumber, &github.IssueListCommentsOptions{
		ListOptions: github.ListOptions{
			PerPage: 100,
		},
	})
	...
	prReviewComments, _, err := h.github.GetClient().PullRequests.ListComments(ctx, owner, repoName, prNumber, &github.PullRequestListCommentsOptions{
		ListOptions: github.ListOptions{
			PerPage: 100,
		},
	})
