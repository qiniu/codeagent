...
func (dm *DirectoryMerger) MergeDirectories() error {
	...
	if err := dm.ensureSubdirectories(); err != nil {
		return fmt.Errorf("failed to create required subdirectories: %w", err)
	}
...
// ensureSubdirectories creates required subdirectories if they don't exist
func (dm *DirectoryMerger) ensureSubdirectories() error {
